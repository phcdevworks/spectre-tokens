...
try {
  ...
  // zIndex/color checks are weaker in that specific sense — a $type: number
  // JS number and a numeric-looking string both render as the same CSS
  // variable text — but they still confirm alias resolution and numeric
  // parsing succeed; check-dtcg-conformance.ts is the check that rejects a
  // zIndex value stored as the wrong JS type.
