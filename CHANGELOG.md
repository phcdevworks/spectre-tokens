...
- `forms.default.{bg,text,placeholder}` are now mode-aware (requested by
  `spectre-ui`, 2026-09-26). Added `modes.default.forms.default.*` and
  `modes.dark.forms.default.*`; the dark values (`neutral.700`/`neutral.100`/
  `neutral.300`) duplicate the literals `surface.input` and
  `component.input.text`/`.placeholder` already use per mode, and the dark
  pair is contrast-checked. `--sp-form-default-{bg,text,placeholder}` are now
  redeclared in the `[data-spectre-theme="dark"]` block instead of cascading
  the light value from `:root`. Top-level `forms.default.*` keeps its light
  value, and `forms.default.border` and every other `forms.*` state are
  unchanged (still `:root` only). `outputParity.css.groups` now checks
  `modes.default.forms` as a `mode-scoped` group, so a regression back to
  light-only fails `check:parity`.
- `typography.display.{1-6}` and `typography.lead` are now emitted as CSS
  custom properties (requested by `spectre-ui`, 2026-09-25). 4.10.0 added
  them to the token source, JS, TypeScript, and DTCG outputs, but
  `src/css.ts` emits typography roles by name and only listed `heading` and
  `body`. New variables follow the heading/body shape:
  `--sp-display-{1-6}-{family,size,line-height,weight,letter-spacing}` and
  `--sp-lead-{family,size,line-height,weight,letter-spacing}`, each a
  resolved literal. `TypographyTokens` now declares `display` and `lead`, and
  `check:typography-refs` validates both roles' references and resolved CSS
  output, so the next role added to `typography.json` without CSS emission
  fails the gate.

### Added

- `DOWNSTREAM_PARITY.md`, generated by `npm run build` from the published CSS
  output: every `--sp-*` variable grouped into one of 69 families (roughly one
  per downstream recipe or stylesheet), with its variable prefixes, count,
  dark-mode coverage, and token source. The build fails if a published
  variable fits no family, so a new token group cannot ship without appearing
  there. `check:dist` now keeps it in sync alongside `TOKEN_REFERENCE.md`.
- `TOKEN_CONTRACT.md` "Contract Expansion Policy" is now proactive (owner
  direction, 2026-09-26): the package grows the token surface ahead of
  downstream demand with one line of intent per addition, replacing the
  evidence gate that required a filed request or audit finding first.
  Guardrails (additive by default, classified breaking changes, protected
  families, token meaning only) are unchanged. Phase 16 (data-viz palette,
  high-contrast mode, control sizing, semantic elevation, selection/caret/
  scrollbar, skeleton, `kbd`) is planned in `TODO.md`.
- `npm run audit:parity [repo]`: read-only report that prints those families
  as a Markdown checklist against a sibling repo (default `spectre-ui`), with
  missing variables and the files that already consume each family. Not part
  of `npm run check`.
